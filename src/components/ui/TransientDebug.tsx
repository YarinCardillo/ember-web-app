...
export function TransientDebug(): JSX.Element {
  ...
  const setAttack = (value: number) => useAudioStore.getState().setParameter('transientAttack', value);
  const setSustain = (value: number) => useAudioStore.getState().setParameter('transientSustain', value);
  const setMix = (value: number) => useAudioStore.getState().setParameter('transientMix', value);
        ...
        <div className="p-3 space-y-3">
          ...
          <div className="space-y-1">
            <div className="flex justify-between items-center text-xs">
              ...
              <span className="text-amber-glow font-mono">{Math.round(attack * 100)}%</span>
            </div>
            <input
              type="range"
              min="-1"
          ...
          <div className="space-y-1">
            <div className="flex justify-between items-center text-xs">
              ...
              <span className="text-amber-glow font-mono">{Math.round(sustain * 100)}%</span>
            </div>
            <input
              type="range"
              min="-1"
          ...
          <div className="space-y-1">
            <div className="flex justify-between items-center text-xs">
              ...
              <span className="text-amber-glow font-mono">{Math.round(mix * 100)}%</span>
            </div>
            <input
              type="range"
              min="0"
