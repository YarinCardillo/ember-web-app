...
export class VinylModeNode {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.pitchShifter = new AudioWorkletNode(this.ctx, 'pitch-shifter-processor');
      
      // Load vinyl buffer worklet (handles variable playback rate)
      this.bufferNode = new AudioWorkletNode(this.ctx, 'vinyl-buffer-processor');
  ...
  private updateRouting(): void {
    ...
    if (this.isBypassed || !this.isInitialized || !this.bufferNode) {
      ...
      try { this.inputGain.connect(this.bypassGain); } catch {}
      ...
      setTimeout(() => {
        if (this.isBypassed) {
          try { this.inputGain.disconnect(this.bufferNode!); } catch {}
        }
      }, crossfadeTime * 1000 + 10);
    } else {
      // Transitioning to active: crossfade from bypass to vinyl
      // First connect both paths
      try { this.inputGain.connect(this.bufferNode); } catch {}
      
      // Fade vinyl mixer in - start at 1.0, compensation will be applied by rampReverbMix
      this.vinylMixerGain.gain.cancelScheduledValues(now);
      this.vinylMixerGain.gain.setValueAtTime(this.vinylMixerGain.gain.value, now);
      this.vinylMixerGain.gain.linearRampToValueAtTime(1.0, now + crossfadeTime);
      ...
      setTimeout(() => {
        if (!this.isBypassed) {
          try { this.inputGain.disconnect(this.bypassGain); } catch {}
        }
      }, crossfadeTime * 1000 + 10);
    }
  }
  ...
  setReverbMix(wet: number): void {
    ...
    this.reverbGain.gain.setValueAtTime(this.currentReverbWet, this.ctx.currentTime);
    ...
    const boostAmount = 1.0 + normalizedWet * (VinylModeNode.REVERB_GAIN_COMPENSATION - 1.0);
  ...
  rampReverbMix(wet: number, durationMs: number): void {
    ...
    this.vinylBoostGain.gain.setValueAtTime(this.vinylBoostGain.gain.value, now);
    
    // Ramp wet/dry mix
    this.reverbGain.gain.linearRampToValueAtTime(this.currentReverbWet, endTime);
    ...
    console.log(`[VinylModeNode] Ramping: wet=${wet}, boost=${boostAmount.toFixed(2)}x`);
  }
