...
class VinylBufferProcessor extends AudioWorkletProcessor {
  constructor() {
    ...
    // At 48kHz, 5 minutes of stereo audio ≈ 28.8MB
    // We'll use 3 minutes as max to be safe
    this.maxBufferSeconds = 180; // 3 minutes
  ...
  process(inputs, outputs, parameters) {
    ...
    this.playbackRate += (this.targetPlaybackRate - this.playbackRate) * this.smoothingFactor;
    ...
    if (backlog > this.bufferSize * 0.9) {
      // Buffer nearly full - force speed up to catch up
    ...
    if (Math.random() < 0.01) { // ~1% of blocks
      this.sendStats();
    }
