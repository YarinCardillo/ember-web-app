...
const LUT_SIZE = 2048;
const LUT_XMAX = 4.0; // table spans grid drive u in [-XMAX, XMAX]; u = 1 -> 0 VU
...
const DYN_BIAS_DEPTH = 0.18; // self-bias drift (cathode cap), per unit level
const DYN_BIAS_TAU = 0.035; // s, slow follower for the bias drift
const BLOCK_THRESH = 2.0; // grid-conduction onset, in u (0 VU = 1)
...
const MILLER_FC = 17000; // Hz, HF rolloff (Miller + coupling) on the high band only
...
const TUBES = {
  "12AU7": { MU: 17, EX: 1.3, KG1: 420, KP: 300, KVB: 300, bias: -4.5, vdrive: 0.4, RLOAD: 22000 },
...
function buildTubeLut(name) {
  ...
  const plateCurrent = (vp, vg) => {
    const grid = vg < GRID_CLAMP ? vg : GRID_CLAMP;
...
class TubeSaturationProcessor extends AudioWorkletProcessor {
  constructor() {
    ...
    this.blockRel = Math.exp(-1 / (BLOCK_TAU * fsOs));
   ...
   */
  _band(st, lut, s, sagScale, applyMiller) {
    const x = s / REFERENCE_LINEAR;
    const ax = x < 0 ? -x : x;
    // self-bias drift: slow follower pulls the operating point with level
    st.dyn += this.dynA * (ax - st.dyn);
    // grid blocking: fast attack when the grid conducts, slow recovery
    const over = ax > BLOCK_THRESH ? ax - BLOCK_THRESH : 0;
    st.block = over > st.block ? over : st.block * this.blockRel;
    const biasShift = -(DYN_BIAS_DEPTH * st.dyn + BLOCK_DEPTH * st.block);
    const y = shape(lut, x + biasShift) * REFERENCE_LINEAR * sagScale;
    if (!applyMiller) return y;
    // Miller capacitance + coupling caps: gentle HF rolloff. Only the high band
    // (the high-gain 12AX7 stage) where Miller actually bites; on low/mid the
    // crossover has already removed everything up here.
    st.miller += this.millerA * (y - st.miller);
    return st.miller;
  }
  ...
  process(inputs, outputs) {
    ...
    for (let c = 0; c < input.length; c++) {
      ...
      for (let j = 0; j < osCount; j++) {
        ...
        os[j] =
          this._band(ch.b0, this.lutLow, low, sagScale, false) +
          this._band(ch.b1, this.lutMid, mid, sagScale, false) +
          this._band(ch.b2, this.lutHigh, high, sagScale, true);
      }
